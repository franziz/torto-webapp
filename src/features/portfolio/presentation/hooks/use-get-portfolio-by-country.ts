...
import { GetPortfolioByCountryUseCase } from "@/features/portfolio/domain/usecases/get-portfolio-by-country.usecases";
import { DataFailed } from "@/core/resources/data-state";
import { ErrorCodes, ServerError } from "@/core/resources/server-error";
import { PortfolioByCountryEntity } from "@/features/portfolio/domain/entities/portfolio-by-country";

type UseGetPortfolioByCountryReturnType =
  | { data: null; loading: true; error: null }
  | { data: PortfolioByCountryEntity[]; loading: false; error: null }
  | { data: null; loading: false; error: ServerError };

async function PortfolioByCountryFetcher([_, clerk]: [string, ReturnType<typeof useClerk>]) {
  ...
  const useCase = new GetPortfolioByCountryUseCase(portfolioRepository, sessionRepository);
...
export function useGetPortfolioByCountry(): UseGetPortfolioByCountryReturnType {
  const clerk = useClerk();
  const { data, isLoading, error } = useSWR(["portfolio-by-country", clerk], PortfolioByCountryFetcher);
