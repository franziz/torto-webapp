...
export class GetPortfolioByAccountUseCase implements UseCase<DataState<PortfolioByAccountEntity[]>, void> {
  constructor(
    private readonly portfolioRepository: PortfolioRepository,
    private readonly sessionRepository: SessionRepository,
  ) {}

  public async execute(): Promise<DataState<PortfolioByAccountEntity[]>> {
    try {
      const session = await this.retrieveSession();
      return this.portfolioRepository.getByAccount(session);
    } catch (err) {
