...
} from "@/features/account/domain/usecases/delete-account.usecases";
...
export function useDeleteAccount() {
  ...
  const fetcher = async (_: string, { arg }: { arg: DeleteAccountUseCaseParams }) => {
    ...
    const useCase = new DeleteAccountUseCase(accountRepository, sessionRepository);
    const result = await useCase.execute(arg);
    if (result instanceof DataFailed) throw result.error;
    return result.data;
  };

  const { trigger, isMutating, error } = useSWRMutation("delete-account", fetcher, {
    onSuccess: () => {
