...
} from "@/features/account/domain/usecases/update-account.usecases";
...
export function useUpdateAccount() {
  ...
  const fetcher = async (_: string, { arg }: { arg: UpdateAccountUseCaseParams }) => {
    ...
    const useCase = new UpdateAccountUseCase(accountRepository, sessionRepository);
    const result = await useCase.execute(arg);
    if (result instanceof DataFailed) throw result.error;
    return result.data;
  };

  const { trigger, isMutating, error } = useSWRMutation("update-account", fetcher, {
    onSuccess: () => {
